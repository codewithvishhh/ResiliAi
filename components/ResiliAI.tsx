...
import {
  ...
  Star, Phone, Settings, Info, Newspaper, ExternalLink, RefreshCw, Clock3, MapPinned
} from "lucide-react";
import {
...
function NewsStatus({ loading, configured, hasArticles, onRefresh }) {
  return (
    <div className="flex items-center gap-2 text-[10px] text-white/40">
      <span className={cx("h-1.5 w-1.5 rounded-full", loading ? "animate-pulse bg-amber-300" : configured && hasArticles ? "bg-emerald-300" : "bg-white/30")} />
      {loading ? "Updating" : configured && hasArticles ? "Updated just now" : "Demo updates"}
      <button onClick={onRefresh} disabled={loading} aria-label="Refresh local news" title="Refresh local news" className="ml-1 rounded-md p-1 text-white/40 transition hover:bg-white/[0.08] hover:text-white disabled:opacity-40">
...
function NewsArticleCard({ item, compact = false }) {
  return (
    <article className={cx("group border border-white/[0.07] bg-white/[0.025] transition hover:border-sky-300/25 hover:bg-white/[0.045]", compact ? "rounded-xl p-3" : "rounded-2xl p-4")}>
      <div className="flex items-start gap-3">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-red-400/10 text-red-200">
          <Newspaper className="h-4 w-4" />
        </div>
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] uppercase tracking-[0.12em] text-white/35">
            <span className="text-red-200/80">{item.type || "Disaster news"}</span>
            <span>·</span>
            <span>{item.area}</span>
            <span className="inline-flex items-center gap-1"><Clock3 className="h-3 w-3" />{formatFeedTime(item.time)}</span>
          </div>
          <h4 className={cx("font-medium leading-5 text-white/90", compact ? "text-[12px]" : "text-[14px]")}>{item.title || item.status}</h4>
          {!compact && item.description && <p className="mt-1.5 line-clamp-2 text-[12px] leading-5 text-white/45">{item.description}</p>}
          {item.url && <a href={item.url} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-[11px] font-medium text-sky-300 transition hover:text-sky-200">Read source <ExternalLink className="h-3 w-3" /></a>}
        </div>
      </div>
    </article>
  );
...
function Overview({ globalRisk, weather, onNavigate, onSelectZone, zones, feedItems, newsLocation, newsLoading, newsConfigured, refreshNews }) {
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          ...
          className="flex shrink-0 items-center gap-2 rounded-full border border-red-400/60 bg-red-500/15 px-4 py-2.5 text-[12px] font-semibold text-red-200 shadow-[0_0_18px_rgba(239,68,68,0.28)] transition-all hover:scale-[1.02] hover:bg-red-500/25 hover:text-white"
        >
          <span className="flex h-7 w-7 items-center justify-center rounded-full bg-red-500/20 text-red-100 shadow-[0_0_18px_rgba(239,68,68,0.5)] ring-2 ring-red-300/60">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.6fr_0.9fr] gap-5">
        ...
        <GlassPanel className="overflow-hidden p-0">
          <div className="border-b border-white/[0.07] bg-gradient-to-r from-red-400/[0.09] to-transparent p-4">
            <div className="mb-3 flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.16em] text-red-300/80"><Newspaper className="h-3.5 w-3.5" /> Local situation</div>
              <h3 className="mt-1 text-[15px] font-semibold text-white">Disaster news near you</h3>
              <p className="mt-1 flex items-center gap-1 text-[11px] text-white/40"><MapPinned className="h-3 w-3" /> {newsLocation}</p>
            </div>
            <NewsStatus loading={newsLoading} configured={newsConfigured} hasArticles={feedItems.some((item) => item.url)} onRefresh={refreshNews} />
            </div>
          </div>

          <div className="space-y-2 p-3">
            {feedItems.length > 0 ? feedItems.slice(0, 3).map((item) => <NewsArticleCard key={item.id} item={item} compact />) : (
              <div className="px-2 py-5 text-center text-[12px] text-white/40">No local disaster stories found right now.</div>
            )}
          </div>
        </GlassPanel>
      </div>
...
/* ---------------------------------------------------------------------
   RISK MAP PAGE
--------------------------------------------------------------------- */
function EmergencyTopicsPage() {
  const topics = [
    {
      title: "Flood safety",
      icon: Droplets,
      description: "Move to higher ground, avoid walking through water, and keep emergency contacts accessible.",
      tips: ["Do not drive through flooded roads", "Switch off utilities if instructed", "Keep essential documents in a waterproof pouch"],
    },
    {
      title: "Medical emergency",
      icon: Stethoscope,
      description: "Call emergency responders immediately for severe injuries, breathing problems, or loss of consciousness.",
      tips: ["Check breathing and pulse", "Keep the person warm and still", "Follow dispatcher instructions exactly"],
    },
    {
      title: "Fire and smoke",
      icon: AlertTriangle,
      description: "Evacuate quickly, stay low, and never use elevators during smoke or fire incidents.",
      tips: ["Close doors behind you", "Use stairwells only", "Call the fire department without delay"],
    },
    {
      title: "Landslide / road block",
      icon: Mountain,
      description: "Avoid unstable slopes, blocked roads, and debris paths when weather conditions are severe.",
      tips: ["Use alternate safe routes", "Stay away from steep terrain", "Report trapped vehicles and hazards"],
    },
    {
      title: "Heat and dehydration",
      icon: Thermometer,
      description: "Rest in cool shaded areas and hydrate regularly to avoid heat exhaustion.",
      tips: ["Wear light clothing", "Drink water frequently", "Avoid direct sun during peak hours"],
    },
    {
      title: "Evacuation checklist",
      icon: Bell,
      description: "Prepare essentials in advance so you can move quickly during a crisis.",
      tips: ["Keep phones, IDs, and medicines ready", "Store water and flashlight", "Know your nearest shelter and route"],
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <p className="text-[12px] uppercase tracking-[0.22em] text-red-300/80">Emergency response</p>
          <h2 className="mt-2 text-[28px] font-semibold text-white" style={{ fontFamily: "Manrope, sans-serif" }}>
            Emergency topics
          </h2>
        </div>
        <button className="flex items-center gap-2 rounded-full border border-red-400/60 bg-red-500/15 px-4 py-2 text-[12px] font-medium text-red-200 shadow-[0_0_20px_rgba(239,68,68,0.25)]">
          <Phone className="w-4 h-4" /> Call 112
        </button>
      </div>

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {topics.map(({ title, icon: Icon, description, tips }) => (
          <GlassPanel key={title} className="p-5">
            <div className="mb-4 flex items-center gap-3">
              <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-red-500/15 text-red-200 ring-1 ring-red-400/40">
                <Icon className="h-5 w-5" />
              </span>
              <h3 className="text-lg font-semibold text-white">{title}</h3>
            </div>
            <p className="text-[13px] leading-6 text-white/65">{description}</p>
...
function CommunityPage({ feedItems, newsLocation, newsLoading, newsConfigured, refreshNews }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <GlassPanel className="overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.07] bg-gradient-to-r from-red-400/[0.09] to-transparent p-5">
          <div>
            <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.16em] text-red-300/80"><Newspaper className="h-3.5 w-3.5" /> Live local intelligence</div>
            <h3 className="mt-1 text-[17px] font-semibold text-white">Disaster news near you</h3>
            <p className="mt-1 flex items-center gap-1 text-[12px] text-white/40"><MapPinned className="h-3 w-3" /> {newsLocation}</p>
          </div>
          <NewsStatus loading={newsLoading} configured={newsConfigured} hasArticles={feedItems.some((item) => item.url)} onRefresh={refreshNews} />
        </div>
        <div className="grid gap-3 p-4 md:grid-cols-2">
          {feedItems.filter((item) => item.url).map((item) => <NewsArticleCard key={item.id} item={item} />)}
          {!feedItems.some((item) => item.url) && <p className="col-span-full py-5 text-center text-[12px] text-white/40">Add a GNews API key to see current location-based disaster news.</p>}
        </div>
      </GlassPanel>
          ...
          <motion.div key={f.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}>
            <GlassPanel className="p-4 flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-[12px] text-white/40 flex items-center gap-1"><MapPin className="w-3 h-3 shrink-0" /> {f.area} · {formatFeedTime(f.time)}</p>
                {f.title && <p className="mt-1 line-clamp-2 text-[12px] text-white/60">{f.title}</p>}
              </div>
...
function DashboardApp({ user }: { user?: FirebaseUser | null }) {
  ...
  const [newsLocation] = useState("Pune Maharashtra");
  ...
  const refreshNews = useCallback(() => {
    setNewsLoading(true);
    api.getNews(newsLocation)
      .then((result) => {
        const articles = result && typeof result === "object" && "articles" in result ? result.articles : [];
        if (Array.isArray(articles) && articles.length > 0) setFeedItems(articles);
        setNewsConfigured(Boolean(result && typeof result === "object" && "configured" in result && result.configured));
      })
      .catch(() => setNewsConfigured(false))
          ...
          <div className="max-w-7xl mx-auto px-5 py-6 pb-24 lg:pb-10 relative z-10">
            ...
            <AnimatePresence mode="wait">
              <motion.div key={active} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}>
                {active === "overview" && <Overview globalRisk={globalRisk} weather={weather} zones={zones} feedItems={feedItems} newsLocation={newsLocation} newsLoading={newsLoading} newsConfigured={newsConfigured} refreshNews={refreshNews} onNavigate={setActive} onSelectZone={setSelectedZone} />}
                {active === "emergency" && <EmergencyTopicsPage />}
                ...
                {active === "community" && <CommunityPage feedItems={feedItems} newsLocation={newsLocation} newsLoading={newsLoading} newsConfigured={newsConfigured} refreshNews={refreshNews} />}
